...
window.addEventListener("load", () => {
	...
	pitLaunchBtn.addEventListener("click", (_) => {
		INVOKE("launch", { page: "pit.html" }).catch((error) => print(error));
	});
	standLaunchBtn.addEventListener("click", (_) => {
		INVOKE("launch", { page: "index.html" }).catch((error) => print(error));
	});

	downloadBtn.addEventListener("click", (_) => {
		print(
			"Note: To get team autocompletion working, you need to launch the stand scouter at least once.",
		);
	...
	cleanCacheBtn.addEventListener("click", (_) => {
		if (cleanCachePresses < 1) {
			cleanCachePresses += 1;
			print(
				"DO NOT RUN THIS IN A COMPETITION, YOU WILL HAVE TO REDOWNLOAD SCOUTER, WHICH REQUIRES WIFI.",
			);
			print(
				"If you really want to clean the cache, hit the clean cache button again.",
			);
		} else {
			INVOKE("clean_cache")
				.then(() =>
					print(
						"Cache cleaned; please redownload scouter to keep using it.",
					),
				)
